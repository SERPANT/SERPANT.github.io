...
const expire = new Date('2026-01-10T12:00:00');
...
function moveElement(element, animeType, pixels) {
  anime({
    targets: element,
    [animeType]: `${pixels}px`,
...
function getRandomNoQuotes() {
  if (NoQuotes.length === 0) {
    return null;
  }

  const index = getRandomNum(NoQuotes.length);
  const message = NoQuotes[index];
  NoQuotes.splice(index, 1);
